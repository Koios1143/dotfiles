...
if command -v upower >/dev/null 2>&1; then
  ...
  if [[ -n "$dev" ]]; then
    bat_percent=$(upower -i "$dev" 2>/dev/null | awk '/percentage:/ {gsub(/%/,"",$2); print $2; exit}' || echo "--")
    state=$(upower -i "$dev" 2>/dev/null | awk '/state:/ {print $2; exit}' || true)
    [[ "$state" == "charging" ]] && charging="true"
    time=$(upower -i "$dev" 2>/dev/null | awk '/time to empty:/ {print $(NF-1),$NF; exit}' || true)
    if [[ -n "$time" ]]; then
...
jq -nc \
  ...
  --arg network "$network" --arg bat "$bat_percent" --argjson bat_seconds "$bat_seconds" --argjson charging "$charging" --arg bt "$bt" --arg kbd "$kbd" \
  '{volume:$vol, muted:$muted, brightness:$bright, cpu:$cpu, gpu:$gpu, ram:$ram, network:$network, battery:$bat, batterySeconds:$bat_seconds, charging:$charging, bluetooth:$bt, keyboard:$kbd}'
